...
int main() {
 ...
 //       "Ber","Jah","Cham","Zod" };
    ...
    fgets(desiredRune_sz, 6, stdin); //Appends user input into above array //may be the issue
    printf("%d\n", sizeof(stdin) / sizeof(stdin[0])); //debug to see if stdin is array
    desiredRune_sz[strlen(desiredRune_sz) - 1] = 0; //Fixed the entire string comparison code. Property of fgets preserves 'enter'
    printf("char 1: %c\n", desiredRune_sz[0]);
    printf("char 3: %c\n", desiredRune_sz[2]);
    ...
    {
        compare_sz = strncmp(runeTable[j], desiredRune_sz, 4); // Compare elemetns 1-2 of char array. Will need to be fixed.
